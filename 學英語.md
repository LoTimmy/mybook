...
A: 有，我喝了二罐啤酒。                                  
B: `All right`, `just a moment`. We are processing your `credit card`.                   
   `Here you are`. `Sign` `at the bottom`, please.                    
B: 好，請等一下。我們正在處理您的信用卡。                                 
   這是帳單，請在下面簽名。 
A: Sure. `Here are` the keys.                                 
A: 好的。這是房間的鑰匙。 


My car engine sounds funny, can you`check it out`? 
我的車子引擎聽起來怪怪的, 你可以檢查一下嗎？ 
...
`There are` some people shooting a movie `over there`, `let's go` `check it out`. 
那裡有人在拍電影呢, 我們過去看一看吧. 

　`New arrival.` Come `check it out!` 
　`新貨到`, 照過來～來看看吧 （可能出現在廣告上的句子） 

`it` 也可以改成其他的代名詞, 如 `her`, `him` 等 

John: `A hot girl` just moved in our next door! 
約翰：一個火辣的女孩剛剛搬到我們隔壁耶 
Andy: `Really?` `I am gonna go` `check her out!` 
安迪：真的嗎？　我要去看看她！ 

但是千萬別當著一個女生的面這麼說喔 
因為這是有一點輕浮與不禮貌的口語說法 
而主題所寫的 `Yo, yo, check it out` 
`Yo, yo` 是黑人說 `you` 的簡稱 
但也可以當成么喝的聲音 
所以可翻成　你～你～　照過來～ 
也可翻成　　呦～呦～　照過來～ 
...
黑人習慣的打呼用語便是`Hi, Man!`及`Yo! What's Up, Man?` 


---

- `Another time…` `改天吧！`
- `I don't care!` `我不管！`   `I don't give a damn!` = `I don't care.` 但是更粗俗一些。
- `What for?` `何必呢？`
- `Nerd.` `書呆子。`
- `Look, not bad, huh?` `不錯吧？`
- `That's terrible!` `真可怕！`
- `You're dreaming!` / `In your dreams.` `別妄想！`
...
一般字在字典上`bomb`是炸彈， 
而現在年輕人常用`She’s da bomb`來形容一個`女生美到炸`！
沒錯！`Bomb`在美國文化中已經成為了一個酷、炫的代用詞， 
形容事情、人或行為超級棒時，我們可以直接說`It’s the bomb!`
The new iPad that `just came out` is `da bomb`!
「新出的iPad實在是太棒了!!!」

`hit the gym`/`club` `上健身房`/`上夜店` 
...
- http://blog.udn.com/kirarin3723/5768576
...
通常對一個`已婚女性`來說， `my in-laws` 就是指夫家的父母－「`公婆`」 
相對的，對一個`已婚男性`而言，這`my in-laws`就是指妻子娘家的父母－「`岳父母`」

當然，自己的父母親永遠都叫做`my parents`任何人在世上為二的爹與娘，無可取代！
...
`in-law`(`姻親`)：`your spouse's family`(`你的配偶的親人`)
`in-laws` = `parents-in-law` ＝ `father-in-law` +  `mother-in-law`
...
##
- `When I was little` `小時候`
- `after work` `下班`

`下班`英文用`get off work`比較貼切一點，除此之外，口語上也常用`after work`來表示`下班`或`下班後`。

Most workers want is to `get off work`.
上班族最想要的就是`下班`。

`Where are you going` `after work` today?
你今天`下班`後要去哪裡？

I have to `go walk the dog` everyday `after work`.
我每天`下班後`都要去`遛狗`。

`I just want` to `get home from work` to `take a bath`.
我現在`只想` `下班回家` `洗澡`。

I want to `see a movie` `after work` today.
我今天`下班後`要去`看電影`。

Where do you usually go `after school`?
通常`放學後`你會去哪裡?

I often `go to my friend's house` `after school`. We `play on-line games` at his house. `It's lots of fun`.
`放學後`，我往往到我朋友的家。我們在他家`玩線上遊戲`。`遊戲很好玩`。


---

- `every morning` `每天上午（晨時）`
- `once a day` `每天一次`
- `once per day` `每天一次`
- `twice daily` `每天兩次`
...
- `every other hour` `每隔1小時；每2小時`
- `every night` `每夜`
- `every hour` `每小時`
- `every day after noon or every evening` `每天下午或每晚`
- `every night at bedtime` `每夜就寢時`
- `every other day` `每隔一天；每兩天`
- `every week` `每周`
- `every four hours` `每四小時`
